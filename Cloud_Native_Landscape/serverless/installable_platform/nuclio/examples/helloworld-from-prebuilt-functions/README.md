...
1. 构建镜像：
   ```shhell
   nuctl build hello-world --platform local --path helloworld.go
   ```
2. 使用 `nuctl deploy` 部署服务：
   ```shell
   nuctl deploy hello-world --run-image nuclio/processor-hello-world:latest \
       --runtime golang \
...
3. 使用 `nuctl invoke` 调用服务：
   ```shell
   nuctl invoke helloworld --platform local --namespace nuclio
